...
if (lowerEqB != "NULL" && upperEqB != "NULL") {
...
#Defining the bounds
if (responseTransform == "none" && EqBtype == "Absolute") {
	lowerEqBT <- lowerEqB 
	upperEqBT <- upperEqB
}
if (responseTransform == "none" && EqBtype == "Percentage") {
	lowerEqBT <- lowerEqB 
	upperEqBT <- upperEqB
}
...
#Generating the alpha level for the TOST test
if (AnalysisType == "Two-sided") {
...
if (AnalysisType == "Two-sided") {
	for (i in 1:tablen) {
		...
		tabs[i,4]=as.numeric(lowerEqB)
		tabs[i,5]=as.numeric(upperEqB)
	...
	for (i in 1:tablen) {
		if(multci$confint[i+2*tablen] < as.numeric(lowerEqB)) {
			tabs[i,6]="Not equivalent"
		}
		if(multci$confint[i+tablen] > as.numeric(upperEqB)) {
			tabs[i,6]="Not equivalent"
		}
		if(multci$confint[i+2*tablen] < as.numeric(upperEqB) && multci$confint[i+tablen] > as.numeric(lowerEqB)) {
			tabs[i,6]="Equivalent"
		}
	}
...
if (AnalysisType == "Upper-sided") {
	...
	for (i in 1:tablen) {
		...
		if(multci$confint[i] > as.numeric(upperEqB)) {
			tabs[i,4]="Not equivalent"
		}
		if(multci$confint[i] < as.numeric(upperEqB) && multci$confint[i+2*tablen] < as.numeric(upperEqB)) {
			tabs[i,4]="Equivalent"
		}
	}
	tabls<-cbind(rows, tabs)
...
if (AnalysisType == "Lower-sided") {
	...
	for (i in 1:tablen) {
		tabs[i,4]="Inconclusive"
		if(multci$confint[i] < as.numeric(lowerEqB)) {
			tabs[i,4]="Not equivalent"
		}
		if(multci$confint[i] > as.numeric(lowerEqB) && multci$confint[i+tablen] > as.numeric(lowerEqB)) {
			tabs[i,4]="Equivalent"
		}
	}
	tabls<-cbind(rows, tabs)
...
if (AnalysisType == "Two-sided") {
	gr_lowerEqB<-as.numeric(lowerEqB)
	gr_upperEqB<-as.numeric(upperEqB)
}
if (AnalysisType == "Lower-sided") {
	gr_lowerEqB<-as.numeric(lowerEqB)
}
...
#===================================================================================================================
	...
	for ( i in 1:dim(tabs3)[1]) {
		...
		if (AnalysisType == "Two-sided") {
			...
			tabs4[i,4]=as.numeric(lowerEqB)
			tabs4[i,5]=as.numeric(upperEqB)
